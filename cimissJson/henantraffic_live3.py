...
def readcldasforlive(ncpath, outpath, udt):
    ...
    #     p=subprocess.Popen('sudo rm -rf '+outfile001,shell=True)
    #     p.wait()
    #写NC文件
    ...
    wunc.close()
    prsnc.close()
    logger.info('/moji/meteo/cluster/program/moge/hnhsr/ncprocess4hnhsr/shell/start_extract.sh '+outfile001)
    p001=subprocess.call('/moji/meteo/cluster/program/moge/hnhsr/ncprocess4hnhsr/shell/start_extract.sh '+outfile001,shell=True)
    logger.info('shell脚本运行状态',p001)
...
def readcimissfordisaster(ncpath, outpath, udt):
    ...
    p=subprocess.call('/moji/meteo/cluster/program/moge/hnhsr/ncprocess4hnhsr/shell/start_extract.sh '+outfile,shell=True)
    logger.info('shell脚本运行状态:/moji/meteo/cluster/program/moge/hnhsr/ncprocess4hnhsr/shell/start_extract.sh '+outfile,p)
    endtime=datetime.datetime.now()
    logger.info((endtime-pdatetime).seconds/3600)
...
def readcldas625forlivebak(ncpath, outpath):
    ...
    #判断文件是否存在，不存在启动备份生产
    bakfile='/moji/meteo/cluster/data/log/bakflag_ele.csv'
    if not os.path.exists(elefullpath):
        #把文件名记录下来，对比准确率
        bakwrfile=open(bakfile,'a')
        bakwrfile.write(elefullpath)
        bakwrfile.close()
        ...
        wd[:]=wdgrid
        dataset.close()
        p03=subprocess.call('/moji/meteo/cluster/program/moge/hnhsr/ncprocess4hnhsr/shell/start_extract.sh ' + outfile001,shell=True)
        logger.info('/moji/meteo/cluster/program/moge/hnhsr/ncprocess4hnhsr/shell/start_extract.sh ' + outfile001,p03)
        wsnc.close()
        prenc.close()
        tempnc.close()
        prsnc.close()
...
def readcimissfordisasterbak(ncpath, outpath):
    ...
    outfile = os.path.join(outpath+'/'+yearstring + '/' + pdatestring,'HNHSR_Meteo_DIS_LIVE_' + start + '_000.nc')
    logger.info(outfile)
    bakfile='/moji/meteo/cluster/data/log/bakflag_dis.csv'
    if not os.path.exists(outfile):
        #记录文件a
        bakwrfile=open(bakfile,'a')
        bakwrfile.write(outfile)
        bakwrfile.close()
        dataset = Dataset(outfile, 'w', format='NETCDF4_CLASSIC')
        dataset.createDimension('lat', 600)
        dataset.createDimension('lon', 700)
        ...
        dataset.close()
        logger.info('dis nc writer finish')
        p04=subprocess.call('/moji/meteo/cluster/program/moge/hnhsr/ncprocess4hnhsr/shell/start_extract.sh ' + outfile,shell=True)
        logger.info('/moji/meteo/cluster/program/moge/hnhsr/ncprocess4hnhsr/shell/start_extract.sh ' + outfile,p04)
        endtime=datetime.datetime.now()
        logger.info((endtime-pdatetime).seconds/3600)
if __name__ == "__main__":
    ...
    formatter = logging.Formatter(
        ...
        '%a, %d %b %Y %H:%M:%S', )
    logfile = os.path.join(logpath,'ele_from20181205.log')
    ...
    # utc03 = -13
    # utc04 = -22
    #readcldasforlive(ncpath, outpath, -12)
    #readcldas625forlivebak(ncpath625,outpath)
    #readcimissfordisasterbak(ncpath625,outpath)
    # 创建后台执行的schedulers
    scheduler = BackgroundScheduler()
    scheduler001=BackgroundScheduler()
    #定义备份
    scheduler002=BackgroundScheduler()
    # 添加调度任务
    # 调度方法为timeTask,触发器选择定时，第一次执行是在每个小时的55分执行。
    scheduler.add_job(readcldasforlive,'cron',minute='55',args=(ncpath, outpath, utc))
    scheduler.add_job(readcimissfordisaster, 'cron', minute='55',
                      args=(ncpath, outpath, utc))
    #第二次执行在下一个小时的47分执行。
    scheduler001.add_job(readcldasforlive,'cron',minute='47',args=(ncpath, outpath, utc01))
    scheduler001.add_job(readcimissfordisaster, 'cron', minute='45',
                      args=(ncpath, outpath, utc01))
    #第三次执行启用备份数据源。在第二个小时的48分执行。
    scheduler002.add_job(readcldas625forlivebak,'cron',minute='48',args=(ncpath625,outpath))
    scheduler002.add_job(readcimissfordisasterbak,'cron',minute='46',args=(ncpath625,outpath))
    scheduler.start()
    scheduler001.start()
    scheduler002.start()
    try:
