...
def readcldasforlive(ncpath, outpath, udt):
    ...
    gust_hn=numpy.array(gust_hn)
    ws_hn=numpy.array(ws_hn)
    rh_hn=numpy.array(rh_hn)
    temp_hn=numpy.array(temp_hn)
    pre_hn=numpy.array(pre_hn)
    wv_hn=numpy.array(wv_hn)
    wu_hn=numpy.array(wu_hn)
    print wv_hn.shape,wu_hn.shape,gust_hn.shape,ws_hn.shape,rh_hn.shape,temp_hn.shape,pre_hn.shape
...
if __name__ == "__main__":
    ...
    utc03 = -11
    utc04 = -22
    #readcldasforlive(ncpath,outpath,utc01)
    # 创建后台执行的schedulers
    ...
    scheduler.add_job(readcldasforlive,'cron',minute='50',max_instances=1,args=(ncpath, outpath, utc01))
    scheduler.add_job(readcimissfordisaster, 'cron', minute='50',max_instances=1,
                      args=(ncpath, outpath, utc01))
    scheduler.add_job(readcldasforlive,'cron',minute='45',max_instances=1,args=(ncpath, outpath, utc02))
    scheduler.add_job(readcimissfordisaster, 'cron', minute='45',max_instances=1,
                      args=(ncpath, outpath, utc02))
    scheduler.add_job(readcldasforlive,'cron',minute='40',max_instances=1,args=(ncpath, outpath, utc03))
    scheduler.add_job(readcimissfordisaster, 'cron', minute='40',max_instances=1,
                      args=(ncpath, outpath, utc03))
    scheduler.add_job(readcldasforlive,'cron',minute='30',max_instances=1,args=(ncpath, outpath, utc04))
    scheduler.add_job(readcimissfordisaster, 'cron', minute='30',max_instances=1,
                      args=(ncpath, outpath, utc04))
    scheduler.start()
    try:
        while True:
            time.sleep(2)
