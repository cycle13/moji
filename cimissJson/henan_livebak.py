...
def ReadCLDASncFile(ncpath, outpath,logpath):
    ...
    odatetime = pdatetime + datetime.timedelta(hours=-10)
    ...
    formatter = logging.Formatter(
        ...
        '%a, %d %b %Y %H:%M:%S', )
    logfile = os.path.join(logpath, 'll' + pdatestring + hourstr + '_'+pdatetimess+'.log')
    ...
    if os.path.exists(outfile):
        filelen=os.path.getsize(outfile)
        if filelen>20*1024*1024:
            logger.info('file exist,2 times return')
            return None
        else:
            os.remove(outfile)
    ...
    #print lightingrid,lightingrid.shape
