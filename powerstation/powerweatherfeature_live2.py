...
description: 设定时间执行
"""
import pygrib,numpy,math,MySQLdb,os,datetime,logging,subprocess,shutil,sys,time
...
def weatherfeatureFromEC(lon,lat,powerstationname,txtpath,starttime):
    ecfilelist=getECfilelistFromsystemtime(starttime,ecrootpath)
    yearstr = starttime.year
    monthstr = starttime.month
    daystr = starttime.day
    hourstr = starttime.hour
    ...
    txtfile = os.path.join(txtpath001, powerstationname +initialtimestring+ '.txt')
    ...
    for i in range(len(perlist_total)):
        ...
        wfile.write(endtimestring+' '+str(perlist_total[i])+'  '+str(perlist_straight[i])+'   '+str(scatteredvalue)+' '+str(perlist_ws[i])+'  '+str(perlist_wd[i])+' '+str(perlist_t[i])+'    '+str(perlist_rh[i])+'  '+str(perlist_p[i]))
        wfile.write('\n')
        #print len(perlist_total),len(perlist_straight),len(perlist_ws),len(perlist_wd),len(perlist_t),len(perlist_p),len(perlist_rh)
        L.append((powerstationname,initial_txt,endtimestring001,str(perlist_total[i]),str(scatteredvalue),str(perlist_straight[i]),str(perlist_ws[i]),str(perlist_wd[i]),str(perlist_t[i]),str(perlist_rh[i]),str(perlist_p[i])))
    wfile.close()
    #数据入库
    print L
    print len(L)
...
if __name__ == "__main__":
    ...
    starttime=datetime.datetime.strptime('2018-10-07 00:00:00','%Y-%m-%d %H:%M:%S')
    ...
    starttime=datetime.datetime.now()
    weatherfeatureFromEC(114.0352, 40.4638, 'TZHCS', txtpath,starttime)
    weatherfeatureFromEC(93.9203, 42.2439, 'XJHMSK', txtpath,starttime)
    weatherfeatureFromEC(101.8403, 24.7708, 'YNCXDZ', txtpath,starttime)
    #定时任务部分
    # 创建后台执行的schedulers
    scheduler = BackgroundScheduler()
    # 添加调度任务
    # 调度方法为timeTask,触发器选择定时，
    scheduler.add_job(weatherfeatureFromEC,'cron',hour='5,17',minute='10',args=(114.0352, 40.4638, 'TZHCS', txtpath,starttime))
    scheduler.add_job(weatherfeatureFromEC,'cron',hour='5,17',minute='20',args=(93.9203, 42.2439, 'XJHMSK', txtpath,starttime))
    scheduler.add_job(weatherfeatureFromEC,'cron',hour='5,17',minute='30',args=(101.8403, 24.7708, 'YNCXDZ', txtpath,starttime))
