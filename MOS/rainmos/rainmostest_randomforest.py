...
将晴雨分类训练的算法改为随机森林，比较和xgboost的效果对比
"""
...
def modelprocess(stationdict, stationlist, ff,allpath,fstr):
    ...
    #晴雨改为随机森林来训练
    rfmodel=RandomForestClassifier(n_estimators=10,max_depth=12,random_state=0,n_jobs=-1)
    ...
    preds01 = model01.predict(xgbtest01, ntree_limit=model01.best_iteration)
    ...
    recall_before=recall_score(y_classorigin,y_test)
    print("训练前的找回率Accuracy: %.2f%%" % (recall_before * 100.0))
    # 分类预报准确率
    print preds
    accuracy_after = accuracy_score(y_test, preds)
    print("训练后的准确率Accuracy: %.2f%%" % (accuracy_after* 100.0))
    recall_after=recall_score(y_test,preds)
    print ("训练后的召回率Accuracy: %.2f%%" % (recall_after* 100.0))
    ...
    time004=time.time()
    logger.info('数据采集（分钟）：'+str((time002-time001)/60)+'模型训练：'+str((time003-time002)/60)+'准确率计算：'+str((time004-time003)/60))
if __name__ == '__main__':
    ...
    print '程序总时间(分钟)：',((endtime - starttime).seconds)/60
