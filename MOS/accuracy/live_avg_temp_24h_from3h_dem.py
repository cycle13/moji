...
description:
"""
import MySQLdb, datetime,math,logging,sys,time
...
def calculate_avg_temp_24h_from3h_accurity(db, initial, pdate, odate, ii):
    ...
    if rows==():
        ...
        return
    #print len(rows)
    sql_live = 'select v01000,AVG(TEM),MAX(TEM_Max),MIN(TEM_Min) from t_r_surf_hour_ele where vdate>"' + pdate + '" and vdate<="' + odate + '" and v_TEM<90 and v_TEM_Max<90 and v_TEM_Min<90 group by v01000;'
    #print sql_live
    cursor = db.cursor()
...
def calculate_temp3h_accurity(db,initial,pdate,odate,ii,logger):
    ...
    db.commit()
def calcaulateAccu10days(initial002):
    initial=initial002+datetime.timedelta(days=-10)
    hourflag=datetime.datetime.now().hour
    if hourflag>12:
        initialstring = datetime.datetime.strftime(initial, '%Y-%m-%d')
        initialstring=initialstring+" 00:00:00"
        initialtime = datetime.datetime.strptime(initialstring, '%Y-%m-%d %H:%M:%S')
    else:
        initial=initial+datetime.timedelta(days=-1)
        initialstring = datetime.datetime.strftime(initial, '%Y-%m-%d')
        initialstring=initialstring+" 12:00:00"
        initialtime = datetime.datetime.strptime(initialstring, '%Y-%m-%d %H:%M:%S')
    ...
    db.close()
def calcaulateAccu72hourtemp(initial001):
    initial=initial001+datetime.timedelta(days=-3)
    hourflag=datetime.datetime.now().hour
    if hourflag>12:
        initialstring = datetime.datetime.strftime(initial, '%Y-%m-%d')
        initialstring=initialstring+" 00:00:00"
    ...
    for u in range(24):
        print u
        pdatetime=initialtime+datetime.timedelta(hours=3*u)
        pdate = datetime.datetime.strftime(pdatetime, '%Y-%m-%d %H:%M:%S')
        odatetime=initialtime+datetime.timedelta(hours=3*(u+1))
        odate = datetime.datetime.strftime(odatetime, '%Y-%m-%d %H:%M:%S')
...
if __name__ == "__main__":
    ...
    initial=initial+datetime.timedelta(days=-4)
    #calcaulateAccu10days(initial)
    #calcaulateAccu72hourtemp(initial)
    # 创建后台执行的schedulers
    scheduler = BackgroundScheduler()
    # 添加调度任务
    # 调度方法为timeTask,触发器选择定时，
    scheduler.add_job(calcaulateAccu10days, 'cron', hour='7,10,19,22', max_instances=1,args=(initial,))
    scheduler.add_job(calcaulateAccu72hourtemp,'cron', hour='7,10,19,22', max_instances=1,args=(initial,))
    try:
        scheduler.start()
        while True:
            time.sleep(2)
