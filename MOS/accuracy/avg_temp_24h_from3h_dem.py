...
def calculate_avg_temp_24h_from3h_accurity(db, initial, pdate, odate, ii):
    ...
    cursor001 = db.cursor()
    sql = 'select city_id,initial_time,avg(temperature),max(temperature),min(temperature),max(temp_max_6h),min(temp_min_6h) from t_r_ec_city_forecast_ele_mos_dem where initial_time="' + initial + '" and forecast_time>"' + pdate + '" and forecast_time<="' + odate + '" group by city_id'
    #print sql
...
def caculate_rainstate24h_from3h_accurity(db, initial, pdate, odate, ii,):
    ...
    sql='select city_id,initial_time,MAX(rainstate) from t_r_ec_city_forecast_ele_mos_dem where initial_time="' + initial + '" and forecast_time>"' + pdate + '" and forecast_time<="' + odate + '" group by city_id'
...
def calculate_rain3h_accurity(db,initial,pdate,odate,ii,logger):
    #取训练的72小时的逐3小时晴雨和降水量
    sql_rain3h='select city_id,initial_time,rainstate,precipitation from t_r_ec_city_forecast_ele_mos_dem where initial_time="' + initial + '" and forecast_time>"' + pdate + '" and forecast_time<="' + odate + '" '
    #print sql_rain3h
...
if __name__ == "__main__":
    ...
    initial = '2018-05-01 12:00:00'
    initialtime = datetime.datetime.strptime(initial, '%Y-%m-%d %H:%M:%S')
    for p in range(31):
        initialtime001=initialtime+datetime.timedelta(hours=24*p)
