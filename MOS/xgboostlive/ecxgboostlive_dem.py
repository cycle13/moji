...
def demdatefromcsvTodict(demcsv):
    ...
    while True:
        ...
        if not line:
            break
    return dictdem
def calculateStationVariable(tempvariablelist,maxtempvariablelist,mintempvariablelist,rainvariablelist,inputfile,stationlist,csvfile,demcsv):
    if inputfile[-4:]=='grib':
        ...
        while True:
            ...
            if not line:
                break
def calculateStationVariable2(tempvariablelist,rainvariablelist,inputfile,stationlist,csvfile,demcsv):
    if inputfile[-4:]=='grib':
        grbs=pygrib.open(inputfile)
        # grbs.seek(0)
        # for grb in grbs:
        ...
        #关闭grib文件
        grbs.close()
def Predict(outfilename,modelname,maxmodel,minmodel,rainmodelfile,premodelfile,tempscalerfile,maxscalerfile,minscalerfile,rainscalerfile,prescalerfile, origintime, foretime, outpath,csvfile,demcsv):
    try:
        ...
        rainvariablelist=[]
        stationlist=[]
        calculateStationVariable(tempvariablelist,maxtempvariablelist,mintempvariablelist,rainvariablelist,outfilename,stationlist,csvfile,demcsv)
        # 加载训练模型
        params = {
            'booster': 'gbtree',
...
def Predict2(outfilename,modelname,rainmodelfile,premodelfile,tempscalerfile,rainscalerfile,prescalerfile, origintime, foretime,csvfile,demcsv):
    try:
        # 取气温、最高气温、最低气温的训练训练矩阵。读文件费劲，连降水一起取了。晴雨、降水一块训
        tempvariablelist = []
        rainvariablelist=[]
        stationlist=[]
        calculateStationVariable2(tempvariablelist,rainvariablelist,outfilename,stationlist,csvfile,demcsv)
        # 加载训练模型
        params = {
            'booster': 'gbtree',
            'objective': 'reg:linear',  # 线性回归
        ...
        for file in files:
            if file[-4:] == '.bz2' and file[:3]=='D1D':
                ...
                if hours%6==0 and hours/6<>0:
                    ...
                    #Predict2(filename,modelname,rainmodelfile,premodelfile,tempscalerfile,rainscalerfile,prescalerfile, origintime, endtime,csvfile,demcsv)
                    pool.apply_async(Predict2,args=(filename,modelname,rainmodelfile,premodelfile,tempscalerfile,rainscalerfile,prescalerfile, origintime, endtime,csvfile,demcsv))
        pool.close()
        pool.join()
