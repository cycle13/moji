...
def calculateStationVariable(hours,tempvariablelist,maxtempvariablelist,mintempvariablelist,rainvariablelist,inputfile,previouspath,stationlist,csvfile):
    if inputfile[-4:]=='grib' and previouspath[-4:]=='grib':
        ...
        while True:
            ...
            if not line:
                break
def calculateStationVariable2(hours,tempvariablelist,rainvariablelist,inputfile,previouspath,stationlist,csvfile):
    if inputfile[-4:]=='grib' and previouspath[-4:]=='grib':
        grbs=pygrib.open(inputfile)
        # grbs.seek(0)
        # for grb in grbs:
        ...
        #关闭grib文件
        grbs.close()
def Predict(hours,outfilename,previouspath,modelname,maxmodel,minmodel,rainmodelfile,premodelfile,tempscalerfile,maxscalerfile,minscalerfile,rainscalerfile,prescalerfile, origintime, foretime, outpath,csvfile):
    try:
        ...
        rainvariablelist=[]
        stationlist=[]
        calculateStationVariable(hours,tempvariablelist,maxtempvariablelist,mintempvariablelist,rainvariablelist,outfilename,previouspath,stationlist,csvfile)
        # 加载训练模型
        params = {
            'booster': 'gbtree',
        ...
        # csvfile = open(csv, 'w')
        sql = 'replace into t_r_ec_city_forecast_ele_mos (city_id,initial_time,forecast_time,forecast_year,forecast_month,forecast_day,forecast_hour,temperature,temp_max_6h,temp_min_6h,rainstate,precipitation)VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'
        L=[]
...
def Predict2(hours,outfilename,previouspath,modelname,rainmodelfile,premodelfile,tempscalerfile,rainscalerfile,prescalerfile, origintime, foretime,csvfile):
    try:
        # 取气温、最高气温、最低气温的训练训练矩阵。读文件费劲，连降水一起取了。晴雨、降水一块训
        tempvariablelist = []
        rainvariablelist=[]
        stationlist=[]
        calculateStationVariable2(hours,tempvariablelist,rainvariablelist,outfilename,previouspath,stationlist,csvfile)
        # 加载训练模型
        params = {
            'booster': 'gbtree',
            'objective': 'reg:linear',  # 线性回归
        ...
        #csv = os.path.join(outpath,origin+'_'+forecast + '.csv')
        #csvfile = open(csv, 'w')
        sql = 'replace into t_r_ec_city_forecast_ele_mos (city_id,initial_time,forecast_time,forecast_year,forecast_month,forecast_day,forecast_hour,temperature,rainstate,precipitation)VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'
        L=[]
        for j in range(len(stationlist)):
            perstationlist = []
            stationid = stationlist[j][0]
...
if __name__ == "__main__":
    ...
    for i in range(len(bz2list001)):
        ...
        if hours>3:
            if i>0:
                previousfile=bz2list[i-1]
            else:
                previousfile=bz2list[i]
        #当前文件
        newfile = file[:-4] + '.grib'
        ...
        rainmodelfile = '/mnt/data/rainscaler/rain' + str(id) + '.model'
        premodelfile = '/mnt/data/rainscaler/pre' + str(id) + '.model'
        rainscalerfile = '/mnt/data/rainscaler/rainscale' + str(id) + '.save'
        prescalerfile = '/mnt/data/rainscaler/prescale' + str(id) + '.save'
        # modelname = '/Users/yetao.lu/Desktop/mos/model/temp/ectt' + str(
        #     id) + '.model'
        # tempscalerfile = '/Users/yetao.lu/Desktop/mos/model/temp/scale' + str(
        #     id) + '.save'
        # rainmodelfile = '/Users/yetao.lu/Desktop/mos/model/pre/rain' + str(id) + '.model'
        # premodelfile = '/Users/yetao.lu/Desktop/mos/model/pre/pre' + str(id) + '.model'
        # rainscalerfile = '/Users/yetao.lu/Desktop/mos/model/pre/rainscale' + str(id) + '.save'
        # prescalerfile = '/Users/yetao.lu/Desktop/mos/model/pre/prescale' + str(id) + '.save'
        ...
        if hours%6==0 and hours/6<>0:
            ...
            minscalerfile='/mnt/data/maxtempscaler/max_scaler' + str(j) + '.save'
            # maxmodel = '/Users/yetao.lu/Desktop/mos/model/maxmintemp/max_temp2.model'
            # minmodel = '/Users/yetao.lu/Desktop/mos/model/maxmintemp/min_temp2.model'
            # maxscalerfile = '/Users/yetao.lu/Desktop/mos/model/maxmintemp/max_scaler2.save'
            # minscalerfile = '/Users/yetao.lu/Desktop/mos/model/maxmintemp/min_scaler2.save'
            ...
            #         csvfile)
            #args= 不能省略
            pool.apply_async(Predict,args=(hours,filename, previouspath,modelname, maxmodel, minmodel,
                    rainmodelfile, premodelfile, tempscalerfile,
                    maxscalerfile, minscalerfile, rainscalerfile,
                    prescalerfile, origintime, endtime, outpath,
                    csvfile))
        else:
            #Predict2(hours,filename,previouspath,modelname,rainmodelfile,premodelfile,tempscalerfile,rainscalerfile,prescalerfile, origintime, endtime,csvfile)
            pool.apply_async(Predict2,args=(hours,filename,previouspath,modelname,rainmodelfile,premodelfile,tempscalerfile,rainscalerfile,prescalerfile, origintime, endtime,csvfile))
    pool.close()
    pool.join()
