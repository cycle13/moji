...
def perstationvalue(vstring,latlonArray,indexlat,indexlon):
    ...
    vstring.append(latlonArray[indexlat][indexlon - 1])
    return vstring
def calculateStationVariable(tempvariablelist,maxtempvariablelist,mintempvariablelist,rainvariablelist,inputfile,stationlist,csvfile):
    if inputfile[-4:]=='grib':
        ...
        while True:
            ...
            if not line:
                break
def calculateStationVariable2(tempvariablelist,rainvariablelist,inputfile,stationlist,csvfile):
    if inputfile[-4:]=='grib':
        grbs=pygrib.open(inputfile)
        # grbs.seek(0)
        # for grb in grbs:
        ...
        while True:
            ...
            if not line:
                break
def Predict(outfilename,modelname,maxmodel,minmodel,rainmodelfile,premodelfile,tempscalerfile,maxscalerfile,minscalerfile,rainscalerfile,prescalerfile, origintime, foretime, outpath,csvfile):
    ...
    rainvariablelist=[]
    stationlist=[]
    calculateStationVariable(tempvariablelist,maxtempvariablelist,mintempvariablelist,rainvariablelist,outfilename,stationlist,csvfile)
    # 加载训练模型
    params = {
        'booster': 'gbtree',
    ...
    for j in range(len(stationlist)):
        ...
        sql = 'insert into t_r_ec_mos_city_forecast_ele (city_id,initial_time,forecast_time,forecast_year,forecast_month,forecast_day,forecast_hour,temperature,temp_max_6h,temp_min_6h,rainstate,precipitation)VALUES ("' + stationid + '","' + origin + '","' + str(
...
def Predict2(outfilename,modelname,rainmodelfile,premodelfile,tempscalerfile,rainscalerfile,prescalerfile, origintime, foretime, outpath,csvfile):
    # 取气温、最高气温、最低气温的训练训练矩阵。读文件费劲，连降水一起取了。晴雨、降水一块训
    tempvariablelist = []
    rainvariablelist=[]
    stationlist=[]
    calculateStationVariable2(tempvariablelist,rainvariablelist,outfilename,stationlist,csvfile)
    # 加载训练模型
    params = {
        'booster': 'gbtree',
        'objective': 'reg:linear',  # 线性回归
    ...
    forecast_minute = foretime.minute
    timestr = datetime.datetime.strftime(origintime, '%Y%m%d%H%M%S')
    csv = os.path.join(outpath, timestr + '_' + origin+'_'+forecast + '.csv')
    csvfile = open(csv, 'w')
    for j in range(len(stationlist)):
        stationid = stationlist[j][0]
        #遍历降水预测中是否有该站点
        ...
        rainstate=rainresult[j]
        # sql='insert into t_r_ec_mos_city_forecast_ele(city_id,initial_time,forecast_time,forecsat_year,forecast_month,forecast_day,forecast_hour,temperature)VALUES ()'
        sql = 'insert into t_r_ec_mos_city_forecast_ele (city_id,initial_time,forecast_time,forecast_year,forecast_month,forecast_day,forecast_hour,temperature,rainstate,precipitation)VALUES ("' + stationid + '","' + origin + '","' + str(
            forecast) + '","' + str(forecast_year) + '","' + str(
            forecast_month) + '","' + str(forecast_day) + '","' + str(
...
if __name__ == "__main__":
    ...
    # 遍历所有文件，预测历史数据
    path='/home/wlan_dev/tmp/20180504'
    ...
    for root, dirs, files in os.walk(path):
        for file in files:
            if file[-5:] == '.grib' and file[:9]=='D1D052912':
                newfile = file[:-5] + '.grib'
                ...
                #     i) + '.save'
                # rainmodelfile = '/Users/yetao.lu/Desktop/mos/model/pre/rain' + str(i) + '.model'
                # premodelfile = '/Users/yetao.lu/Desktop/mos/model/pre/pre' + str(i) + '.model'
                # rainscalerfile = '/Users/yetao.lu/Desktop/mos/model/pre/rainscale' + str(i) + '.save'
                # prescalerfile = '/Users/yetao.lu/Desktop/mos/model/pre/prescale' + str(i) + '.save'
                rainmodelfile = '/mnt/data/rainmodel/rain' + str(i) + '.model'
                premodelfile = '/mnt/data/rainmodel/pre' + str(i) + '.model'
                rainscalerfile = '/mnt/data/rainmodel/rainscale' + str(i) + '.save'
                prescalerfile = '/mnt/data/rainmodel/prescale' + str(i) + '.save'
                if not os.path.exists(modelname):
                    modelname='/mnt/data/tempscaler/ectt1.model'
                if not os.path.exists(tempscalerfile):
                    tempscalerfile='/mnt/data/tempscaler/scale1.save'
                if not os.path.exists(rainmodelfile):
                    rainmodelfile='/mnt/data/rainmodel/rain1.model'
                if not os.path.exists(premodelfile):
                    premodelfile='/mnt/data/rainmodel/pre1.model'
                if not os.path.exists(rainscalerfile):
                    rainscalerfile='/mnt/data/rainmodel/rainscale1.save'
                if not os.path.exists(prescalerfile):
                    prescalerfile='/mnt/data/rainmodel/prescale1.save'
                ...
                if hours%6==0 and hours/6<>0:
                    ...
                    j = hours / 6
                    j=j-1
                    maxmodel='/mnt/data/maxtempscaler/max_temp' + str(j) + '.model'
                    minmodel='/mnt/data/maxtempscaler/min_temp' + str(j) + '.model'
                    ...
                    #     j) + '.model'
                    maxscalerfile='/mnt/data/maxtempscaler/max_scaler' + str(j) + '.save'
                    minscalerfile='/mnt/data/mintempscaler/max_scaler' + str(j) + '.save'
                    ...
                    #     j) + '.save'
                    if not os.path.exists(maxmodel):
                        maxmodel='/mnt/data/maxtempscaler/max_temp1.model'
                    if not os.path.exists(minmodel):
                        minmodel='/mnt/data/maxtempscaler/min_temp1.model'
                    if not os.path.exists(maxscalerfile):
                        maxscalerfile='/mnt/data/maxtempscaler/max_scaler1.save'
                    if not os.path.exists(minscalerfile):
                        minscalerfile = '/mnt/data/maxtempscaler/max_scaler1.save'
                    ...
                    Predict(filename, modelname, maxmodel, minmodel,
                            ...
                            csvfile)
                    # pool.apply_async(Predict,(filename, modelname, maxmodel, minmodel,
                    #         rainmodelfile, premodelfile, tempscalerfile,
                    #         maxscalerfile, minscalerfile, rainscalerfile,
                    #         prescalerfile, origintime, endtime, outpath,
                    #         csvfile))
                    ...
                    flag=1
                    Predict2(filename,modelname,rainmodelfile,premodelfile,tempscalerfile,rainscalerfile,prescalerfile, origintime, endtime, outpath,csvfile)
        #             pool.apply_async(Predict2,args=(filename,modelname,rainmodelfile,premodelfile,tempscalerfile,rainscalerfile,prescalerfile, origintime, endtime, outpath,csvfile))
        # pool.close()
        # pool.join()
