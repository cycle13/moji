...
def Predict(outfilename,modelname,maxmodel,minmodel,rainmodelfile,premodelfile,tempscalerfile,maxscalerfile,minscalerfile,rainscalerfile,prescalerfile, origintime, foretime, outpath,csvfile):
    try:
        ...
        # csvfile = open(csv, 'w')
        sql = 'insert into t_r_ec_city_forecast_ele_mos (city_id,initial_time,forecast_time,forecast_year,forecast_month,forecast_day,forecast_hour,temperature,temp_max_6h,temp_min_6h,rainstate,precipitation)VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'
        L=[]
        ...
        logger.info(outfilename)      # 取气温、最高气温、最低气温的训练训练矩阵。读文件费劲，连降水一起取了。晴雨、降水一块训
        tempvariablelist = []
        maxtempvariablelist = []
        mintempvariablelist = []
        rainvariablelist=[]
...
def Predict2(outfilename,modelname,rainmodelfile,premodelfile,tempscalerfile,rainscalerfile,prescalerfile, origintime, foretime,csvfile):
    try:
        # 取气温、最高气温、最低气温的训练训练矩阵。读文件费劲，连降水一起取了。晴雨、降水一块训
        tempvariablelist = []
        rainvariablelist=[]
...
if __name__ == "__main__":
    ...
    for root, dirs, files in os.walk(path):
        for file in files:
            #print file
            if file[-4:] == '.bz2' and file[:17]=='D1D04090000040909':
                newfile = file[:-4] + '.grib'
                ...
                rainmodelfile = '/mnt/data/rainscaler/rain' + str(i) + '.model'
                premodelfile = '/mnt/data/rainscaler/pre' + str(i) + '.model'
                rainscalerfile = '/mnt/data/rainscaler/rainscale' + str(i) + '.save'
                prescalerfile = '/mnt/data/rainscaler/prescale' + str(i) + '.save'
                #6小时最高气温和最低气温ID获取，判断数据里有没有6小时最高最低气温要素
                if hours%6==0 and hours/6<>0:
                    #初始场数据中也没有6小时预报
                    j = hours / 6
                    j=j-1
                    logger.info('j='+str(j)+'i='+str(i)+'hours='+str(hours))
                    ...
                    Predict2(filename,modelname,rainmodelfile,premodelfile,tempscalerfile,rainscalerfile,prescalerfile, origintime, endtime,csvfile)
