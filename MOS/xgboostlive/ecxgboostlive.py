...
def Predict(outfilename,modelname,maxmodel,minmodel,rainmodelfile,premodelfile,tempscalerfile,maxscalerfile,minscalerfile,rainscalerfile,prescalerfile, origintime, foretime, outpath,csvfile):
    ...
    for j in range(len(stationlist)):
        ...
        sql = 'insert into t_r_ec_mos_city_forecast_ele_mos (city_id,initial_time,forecast_time,forecast_year,forecast_month,forecast_day,forecast_hour,temperature,temp_max_6h,temp_min_6h,rainstate,precipitation)VALUES ("' + stationid + '","' + origin + '","' + str(
...
#不被6整除的时次没有6h最高气温最低气温要素，本来想写在一个函数里，但预报要素多，为了提高效率还是分开，后期可以再加3h的最高最低
def Predict2(outfilename,modelname,rainmodelfile,premodelfile,tempscalerfile,rainscalerfile,prescalerfile, origintime, foretime, outpath,csvfile):
    ...
    for j in range(len(stationlist)):
        ...
        rainstate=rainresult[j]
        # sql='insert into t_r_ec_mos_city_forecast_ele(city_id,initial_time,forecast_time,forecsat_year,forecast_month,forecast_day,forecast_hour,temperature)VALUES ()'
        sql = 'insert into t_r_ec_mos_city_forecast_ele_mos (city_id,initial_time,forecast_time,forecast_year,forecast_month,forecast_day,forecast_hour,temperature,rainstate,precipitation)VALUES ("' + stationid + '","' + origin + '","' + str(
            forecast) + '","' + str(forecast_year) + '","' + str(
            forecast_month) + '","' + str(forecast_day) + '","' + str(
