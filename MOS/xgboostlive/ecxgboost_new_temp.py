...
def calculatePerStationVariable(inputfile,longitude,latitude):
    if inputfile[-4:]=='grib':
        ...
        vstring=[]
        names=['2 metre temperature','2 metre dewpoint temperature','10 metre U wind component','10 metre V wind component','Total cloud cover','Low cloud cover','Relative humidity']
        #各要素---气温
        for name in names:
...
def Predict(outfilename, modelname, origintime, n, outpath):
    ...
    while True:
        ...
        if len(perlist) >= 4:
            ...
            perstationlist=calculatePerStationVariable(newfile,float(perlist[2]),float(perlist[1]))
        if not line:
            break
        # idlist和ecvaluelist是一一对应的，list是有顺序的
        idlist.append(perlist[0])
