...
description:
"""
import logging,os,sys,multiprocessing,Nio
...
def main():
    ...
    lock=manager.Lock()
    titles=manager.dict()
    titlevs=manager.list()
    num=1
    sfc_varinames = ['CP_GDS0_SFC', 'TP_GDS0_SFC']
    sfc_varinames1 = ['10U_GDS0_SFC', '10V_GDS0_SFC', '2T_GDS0_SFC',
        '2D_GDS0_SFC']
    pl_varinames = ['R_GDS0_ISBL', 'U_GDS0_ISBL', 'V_GDS0_ISBL']
    #开启进程池
    pool=multiprocessing.Pool(processes=24)
    for rootpath, dirs, files in os.walk(inputpath):
        for file in files:
            if file[:3] == 'sfc' and file[-5:] == '.grib':
                inputfile = os.path.join(rootpath, file)
                inputfile2 = inputfile.replace('sfc', 'pl')
                sfcfile = Nio.open_file(inputfile, 'r')
                plfile = Nio.open_file(inputfile2, 'r')
                pool.apply_async(target=GetStationsAndOnetimesFromEC,args=(num,sfc_varinames,sfc_varinames1,sfcfile,pl_varinames,plfile,))
