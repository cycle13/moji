...
"""
import Nio, datetime, os, xgboost, numpy, math, string,multiprocessing,logging,sys
...
def GetOnetimeFromEC(n, max_varinames, h3file, sfc_varinames1, sfc_file,
                     pl_varinames, pl_file, indexlat, indexlon,
                     trainlabellist):
    vstring = []
    ...
    for i in range(len(max_varinames)):
        ...
        print len(variArray)
        #这里数组有40层，索引从0-39，当外面为第一次预报时，这里索引为0。外面的取值是从1-40；
        # for aa in range(len(variArray)):
        #     print variArray[aa]
        if n-1<0:
            return
...
def GetStationsAndOnetimesFromEC(i, max_varinames, h3file, sfc_varinames1,
    ...
    #print 'ecvaluelist', len(ecvaluelist), 'maxtemplist', len(
        #maxtemplist), 'mintemplist', len(mintemplist)
# EC格点数据的获取
def modelprocess(stationdict, stationlist, ll, allpath,forehours):
    logname = 'maxminlog' + str(forehours) + ".out"
    ...
    if ll==0:
        max_varinames=['MX2T6_GDS0_SFC', 'MN2T6_GDS0_SFC']
    dict01 = {}
    # 遍历文件
    ecvaluelist = []
    maxtemplist = []
    ...
    #print '打印' ,ecvaluelist.shape
    max_scaler=preprocessing.StandardScaler().fit(maxecvaluelist)
    max_scaled=max_scaler.transform(maxecvaluelist)
    maxecvaluelist=max_scaled
    maxscaler_file=os.path.join(outpath, 'max_scaler' + str(forehours) + '.save')
    joblib.dump(max_scaler,maxscaler_file)
    min_scaler=preprocessing.StandardScaler().fit(minecvaluelist)
    min_scaled=min_scaler.transform(minecvaluelist)
    minecvaluelist=min_scaled
    minscaler_file=os.path.join(outpath, 'min_scaler' + str(forehours) + '.save')
    joblib.dump(min_scaler,minscaler_file)
    # xgboost，训练集和预测集分割
    ...
    u_train, u_test, v_train, v_test = train_test_split(minecvaluelist,
                                                        ...
                                                        test_size=0.33,
                                                        random_state=7)
    # print len(x_train), len(x_test), len(y_train), len(y_test), len(
    #     u_train), len(u_test), len(v_train), len(v_test)
    ...
    model.save_model(os.path.join(outpath, 'max_temp' + str(forehours) + '.model'))
    model01.save_model(os.path.join(outpath, 'min_temp' + str(forehours) + '.model'))
    # print preds, preds01
    # 训练后高温的RMSE，MAE
    ...
    # 训练前低温的RMSE,MAE
    class_test = min_test.astype('float64')
    min_ec =(class_test[:, 0]) - 273.15
    ...
    print  str(rmse) + ',' + str(mae) + ',' + str(accuracy2_after) + ',' + str(
        ...
        min_accuracy2_before) + ',' + str(min_accuracy3_before)
...
if __name__ == '__main__':
    ...
    #outpath = '/Users/yetao.lu/Desktop/mos/testdata'
    ...
    for ll in range(1,41,1):
        ...
        #print 'a',len(stationdict)
        #allpath = '/Users/yetao.lu/Desktop/mos/testdata'
        allpath = '/mnt/data/MOS/'
        #modelprocess(stationdict, stationlist, ll, allpath,forehours)
        pool.apply_async(modelprocess,args=(stationdict, stationlist, ll, allpath,forehours))
    pool.close()
    pool.join()
