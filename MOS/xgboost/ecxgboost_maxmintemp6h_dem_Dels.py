...
因此6小时修改，时次、文件，和时间间隔
#增加位势高度和DEM数据因子，2018年
#2018-04-17删除站点经度纬度和海拔高度
...
def modelprocess(stationdict, stationlist, ll, allpath,dempath):
    ...
    # 为了统计准确率，提前分割数据集
    class_train, class_test = train_test_split(ecvaluelist, test_size=0.33,
                                               random_state=7)
    ...
    # xgboost，训练集和预测集分割
    x_train, x_test, y_train, y_test = train_test_split(ecvaluelist,
                                                        maxtemplist,
                                                        test_size=0.33,
                                                        random_state=7)
    u_train, u_test, v_train, v_test = train_test_split(ecvaluelist,
                                                        mintemplist,
                                                        test_size=0.33,
                                                        random_state=7)
    ...
    print  str(rmse) + ',' + str(mae) + ',' + str(accuracy2_after) + ',' + str(
        ...
        accuracy2_before) + ',' + str(accuracy3_before) + ','+str(bias)+','+str(bias0)+',' + str(
        min_rmse) + ',' + str(min_mae) + ',' + str(
        min_accuracy2_after) + ',' + str(min_accuracy3_after) + ',' + str(
        min_rmse0) + ',' + str(min_mae0) + ',' + str(
        min_accuracy2_before) + ',' + str(min_accuracy3_before)+ ',' + str(min_bias)+ ',' + str(min_bias0)
...
if __name__ == '__main__':
    ...
    #vioutpath = '/Users/yetao.lu/Desktop/mos/anonymous'
