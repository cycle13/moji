...
"""
import Nio, datetime, os, string, logging, xgboost, numpy, math, sys, \
    multiprocessing
...
def GetStationsAndOnetimesFromEC(i, sfc_varinames, sfc_file, inputfile,
                                 stationsVlist, trainlebellist, stationdict,
...
def modelprocess(stationdict, stationlist, ll):
            ...
            if file[:3] == 'sfc' and file[-5:] == '.grib':
                ...
                try:
                    GetStationsAndOnetimesFromEC(ll, sfc_varinames, sfcfile,
                                                 inputfile, stationsVlist,
                                                 trainlebellist, stationdict,
                                                 stationlist, dict01,
                                                 trainclasfierlist,
                                                 rainvaluelist, rainllabelist)
                except Exception, ex:
    ...
    # xgboost，训练集和预测集分割
    x_train, x_test, y_train, y_test, u_train, u_test, v_train, v_test = train_test_split(
        stationArray, trainlebelArray, rainvaluelist, rainllabelist,
    ...
    model01 = xgboost.train(plst, xgbtrain01, num_rounds, watchlist,
                            early_stopping_rounds=500)
