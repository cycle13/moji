...
def InterpolateGetOnetimeFromEC(n, sfc_varinames, sfc_file, longitude, latitude,
    ...
    return vstring
def get_nearest_point(_city_lon, _city_lat,origin_lon,termination_lat,cellsize):
    #获取城市所在格子的四个角的顶点
    print _city_lon, _city_lat,origin_lon,termination_lat,cellsize
    indexlon=int(_city_lon-origin_lon)/cellsize
    indexlat=int(termination_lat-_city_lat)/cellsize
    ...
    r1 = get_spherical_distance(_city_lat, _city_lon, lat1,
                                lon1)
    r2 = get_spherical_distance(_city_lat, _city_lon, lat2,
                                lon2)
    r3 = get_spherical_distance(_city_lat, _city_lon, lat3,
                                lon4)
    r4 = get_spherical_distance(_city_lat, _city_lon, lat4,
                                lon4)
    print r1,r2
...
def interpolate_grid_to_city(_var_grid, _mode, *_nearest_point_info):
    ...
    if _mode == 0:
        # 反距离加权插值
        print 'value',len(_var_grid),len(_var_grid[i])
...
if __name__=='__main__':
    ...
    outpath='/Users/yetao.lu/Desktop/mos'
    ...
    results=[]
    ii=0
    modelprocess(stationdict,stationlist,ii)
