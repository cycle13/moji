...
author:     yetao.lu
date:       2018/1/30
description:  该方法是65次循环，对65次预报时次进行训练，
2018-04-17增加位势高度和DEM数据
"""
...
def DemProcessing(dempath,latitude,longitude,vstring):
    ...
    vstring.append(myarray[indexlat][indexlon])
    return vstring
def GetOnetimeFromEC(n,sfc_varinames,sfc_file,latitude,longitude,pl_varinames,pl_file):
    vstring=[]
    # #    #首先计算经纬度对应格点的索引，
    ...
    return vstring
def GetStationsAndOnetimesFromEC(i,sfc_varinames,sfc_file,pl_varinames,pl_file,inputfile,stationlist,stationdict):
    ...
    for j in range(len(stationlist)):
        ...
        if trainlebel<>None and trainlebel<>999999:
            ...
            perstationlist=GetOnetimeFromEC(i,sfc_varinames,sfc_file,latitude,longitude,pl_varinames,pl_file)
...
if __name__ == '__main__':
    ...
    stationlist=[]
    dict={}
    stationdict=Getstationdict(tempcsv)
    #站点列表数据
    ...
    for rootpath, dirs, files in os.walk(allpath):
        for file in files:
            if file[:3] == 'sfc' and file[-5:] == '.grib' and (string.find(file,'2014')==-1):
                ...
                GetStationsAndOnetimesFromEC(ll,sfc_varinames,sfcfile,pl_varinames,plfile,inputfile,stationlist,stationdict)
    ...
    print str(rmse0)+','+str(mae0)+','+str(accuracy2_before)+','+str(accuracy3_before)+','+str(bias0)+','+str(rmse)+','+str(mae)+','+str(accuracy2_after)+','+str(accuracy3_after)+','+str(bias)+','+str(len(a_train))+','+str(len(a_test))
    model.save_model('/home/wlan_dev/model/ect'+str(ll)+'.model')
    testfile='/home/wlan_dev/model/test'+str(ll)+'.csv'
    predsfile='/home/wlan_dev/model/preds'+str(ll)+'.csv'
    originfile='/home/wlan_dev/model/origin'+str(ll)+'.csv'
