...
def GetOnetimeFromEC(n, sfc_varinames,sfc_varinames1, sfc_file,pl_varinames,pl_file, indexlat, indexlon):
    ...
    #cp,tp两个变量比较特殊，是累计，需要减去前一个时次的,两者的单位都是米，所以*1000换算成毫米
...
def modelprocess(stationdict, stationlist, ll,allpath):
    ...
    rainscalefile=os.path.join(outpath,'rainscale' + str(ll) + '.save')
    ...
    prescalefile=os.path.join(outpath,'prescale' + str(ll) + '.save')
    ...
    model.save_model(os.path.join(outpath,'rain' + str(ll) + '.model'))
    model01.save_model(os.path.join(outpath,'pre' + str(ll) + '.model'))
    #print preds, preds01
    na=0
    nb=0
    nc=0
