...
author:     yetao.lu
date:       2018/1/30
description:  对降水先进行晴雨训练，然后再进行降水量进行训练,该程序只对降水因子进行。
...
def GetOnetimeFromEC(n, sfc_varinames, sfc_file, indexlat, indexlon):
    ...
    for i in range(len(sfc_varinames)):
        ...
        #print vstring
    return vstring
def GetStationsAndOnetimesFromEC(i, sfc_varinames, sfc_file, inputfile,
...
def modelprocess(stationdict, stationlist, ll):
    ...
    #print stationArray
    #晴雨预测集
    trainlebelArray = numpy.array(trainclasfierlist)
    #print trainlebelArray
    #降水训练集
    ...
    # xgboost，训练集和预测集分割，x矩阵为要素矩阵，数值为原值，y矩阵为晴雨矩阵，数据为0，1.
    ...
    print str(n)+','+str(na)+','+str(nb)+','+str(nc)+','+str(nd)+','+str(na01)+','+str(nb01)+','+str(nc01)+','+str(nd01)+','+str(accuracy_before)+','+str(accuracy_after)+','+str(ts)+','+str(ts01)+','+str(bias)+','+str(bias01)+','+str(ets)+','+str(ets01)
    #对模型1的检测样本进行输出
    print len(classecll),len(y_classorigin),len(preds),len(y_test)
    ...
    print("训练后MAE: %.4f" % mae)
    # 和EC中原始数据对比获取均方误差,这里不能取u_test,u_test是标准化了的数据集
    y_origin = a_test[:, 0]
    print len(a_test),len(u_test),len(v_test)
    ...
    rainfw.close()
    #打印所有变量
    print str(n)+','+str(na)+','+str(nb)+','+str(nc)+','+str(nd)+','+str(na01)+','+str(nb01)+','+str(nc01)+','+str(nd01)+','+str(accuracy_before)+','+str(accuracy_after)+','+str(ts)+','+str(ts01)+','+str(bias)+','+str(bias01)+','+str(ets)+','+str(ets01)+','+str(rmse0)+','+str(mae0)+','+str(rmse)+','+str(mae)+','+str(len(u_train))+','+str(len(u_test))
if __name__ == '__main__':
    ...
    modelprocess(stationdict,stationlist,ll)
