...
2018-05-28:修改只有晴雨两类预报,并且EC的降水阈值取1.87；
"""
import Nio, datetime, os, xgboost, numpy, math,pandas
    ...
    for i in range(len(sfc_varinames)):
        ...
        if i==0:
            if 1000 * (
                ...
                vstring.append(0)
            if 1000 * (latlonArray[indexlat][indexlon + 1] -
                                   platlonArray[indexlat][indexlon + 1])-1.87>0:
                vstring.append(1000 * (latlonArray[indexlat][indexlon + 1] -
                                       platlonArray[indexlat][indexlon + 1])-1.87)
            else:
                vstring.append(0)
            if 1000 * (latlonArray[indexlat + 1][indexlon + 1] -
                                   platlonArray[indexlat + 1][indexlon + 1])-1.87>0:
                vstring.append(1000 * (latlonArray[indexlat + 1][indexlon + 1] -
                                       platlonArray[indexlat + 1][indexlon + 1])-1.87)
            else:
                vstring.append(0)
            if 1000 * (latlonArray[indexlat + 1][indexlon] -
                                   platlonArray[indexlat + 1][indexlon])-1.87>0:
                vstring.append(1000 * (latlonArray[indexlat + 1][indexlon] -
                                       platlonArray[indexlat + 1][indexlon])-1.87)
            else:
                vstring.append(0)
            if 1000 * (latlonArray[indexlat - 1][indexlon - 1] -
                                   platlonArray[indexlat - 1][indexlon - 1])-1.87>0:
            ...
            if 1000 * (latlonArray[indexlat - 1][indexlon + 2] -
                                   platlonArray[indexlat - 1][indexlon + 2])-1.87>0:
                vstring.append(1000 * (latlonArray[indexlat - 1][indexlon + 2] -
                                       platlonArray[indexlat - 1][indexlon + 2])-1.87)
            else:
                vstring.append(0)
            if 1000 * (latlonArray[indexlat][indexlon + 2] -
                                   platlonArray[indexlat][indexlon + 2])-1.87>0:
                vstring.append(1000 * (latlonArray[indexlat][indexlon + 2] -
                                       platlonArray[indexlat][indexlon + 2])-1.87)
            else:
                vstring.append(0)
            if 1000 * (latlonArray[indexlat + 1][indexlon + 2] -
                                   platlonArray[indexlat + 1][indexlon + 2])-1.87>0:
            ...
            if 1000 * (latlonArray[indexlat + 2][indexlon + 2] -
                                   platlonArray[indexlat + 2][indexlon + 2])-1.87>0:
            ...
            if 1000 * (latlonArray[indexlat + 2][indexlon + 1] -
                                   platlonArray[indexlat + 2][indexlon + 1])-1.87>0:
                vstring.append(1000 * (latlonArray[indexlat + 2][indexlon + 1] -
                                       platlonArray[indexlat + 2][indexlon + 1])-1.87)
            else:
                vstring.append(0)
            if 1000 * (latlonArray[indexlat + 2][indexlon] -
                                   platlonArray[indexlat + 2][indexlon])-1.87>0:
                vstring.append(1000 * (latlonArray[indexlat + 2][indexlon] -
                                       platlonArray[indexlat + 2][indexlon])-1.87)
            else:
                vstring.append(0)
            if 1000 * (latlonArray[indexlat + 2][indexlon - 1] -
                                   platlonArray[indexlat + 2][indexlon - 1])-1.87>0:
...
def modelprocess(stationdict, stationlist, ll,allpath):
    sfc_varinames = ['TP_GDS0_SFC','CP_GDS0_SFC']
    ...
    # 晴雨训练集
    #print stationsVlist
    stationsVlist = numpy.array(stationsVlist)
    #print stationArray
    #晴雨预测集
    trainclasfierlist = numpy.array(trainclasfierlist)
    #print trainclasfierlist.shape,stationsVlist.shape
    #重采样数据集，使得晴雨数据集均衡
    rus=RandomUnderSampler()
    x_resampled,y_resampled=rus.fit_sample(stationsVlist,trainclasfierlist)
    ...
    #降水预测集
    rainllabelist=numpy.array(rainllabelist)
    #print rainllabelArray
    #为了统计准确率，提前分割晴雨数据集
    class_train,class_test=train_test_split(x_resampled,test_size=0.2,random_state=7)
    a_train, a_test = train_test_split(rainvaluelist, test_size=0.2,random_state=7)
    #数据训练前进行标准化
    ...
    rainscalefile=os.path.join(outpath,'x_rainscale' + str(ll) + '.save')
    ...
    prescalefile=os.path.join(outpath,'x_prescale' + str(ll) + '.save')
    ...
    u_train, u_test, v_train, v_test=train_test_split(rainvalueArray, rainllabelist,test_size=0.2, random_state=7)
    ...
    model.save_model(os.path.join(outpath,'x_rain' + str(ll) + '.model'))
    model01.save_model(os.path.join(outpath,'x_pre' + str(ll) + '.model'))
    #print preds, preds01
    na=0
    nb=0
    nc=0
    ...
    classfile=os.path.join(outpath,'x_class'+str(ll)+'.csv')
    ...
    predsfile = os.path.join(outpath, 'x_preds' + str(ll) + '.csv')
    ...
    #输出降水量的预报
    rainfile = os.path.join(outpath, 'x_rainpreds' + str(ll) + '.csv')
