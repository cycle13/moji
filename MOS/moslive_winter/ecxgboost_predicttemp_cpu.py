...
description:
"""
import logging,sys,datetime,os,pygrib,numpy,xgboost,MySQLdb,multiprocessing
...
if __name__ == "__main__":
    ...
    # 文件日志learning
    logfile = '/home/wlan_dev/winter_cpu.log'
    ...
    starttime = datetime.datetime.strptime('2018-10-01 17:00:00',
                                           '%Y-%m-%d %H:%M:%S')
    # for i in range(30):
        # starttime=starttime_0+datetime.timedelta(hours=i*12)
    ...
    ecpool=multiprocessing.Pool(processes=20)
    for i in range(len(sortecfilelist)):
        ...
        if os.path.exists(modelfile) and os.path.exists(scalefile):
            ...
            ecpool.apply_async(predictmodel,args=(file,filefullpath,modelfile,scalefile,stationlist,demdict,origintime,endtime))
    ecpool.close()
    ecpool.join()