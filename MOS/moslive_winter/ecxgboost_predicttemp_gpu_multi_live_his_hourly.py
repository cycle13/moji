...
def readymodel(starttime001,path):
    ...
    for i in range(len(sortecfilelist)):
        ...
        if os.path.exists(modelfile) and os.path.exists(scalefile):
            ...
            if int(id)<=90:
                ...
                pool001.apply_async(predictmodel,args=(file,filefullpath,modelfile,scalefile,stationlist,demdict,origintime,endtime,'2'))
            elif int(id)>90:
                pool002.apply_async(predictmodel,args=(file,filefullpath,modelfile,scalefile,stationlist,demdict,origintime,endtime,'3'))
    pool001.close()
    pool002.close()
    pool001.join()
    pool002.join()
    ...
    #新EC数据前90个预报时次是逐小时的D2S开头，中间18个预报时次是逐3小时的，后面16个预报时次是6小时的
    for u in range(124):
        if u <90:
