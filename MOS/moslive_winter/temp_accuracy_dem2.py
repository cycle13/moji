...
description:
"""
import MySQLdb, datetime,math,sys
'''
...
def calculate_avg_temp_24h_from3h_accurity(db, initial, pdate, odate, ii):
    ...
    sql = 'select city_id,initial_time,avg(temperature),max(temperature),min(temperature) from t_r_ec_city_forecast_ele_mos_dem_winter where initial_time="' + initial + '" and forecast_time>"' + pdate + '" and forecast_time<="' + odate + '" group by city_id'
    print sql
    ...
    for row in rows:
        if row[0] in rowdict.keys():
            ...
            if abs(avg_temp_live - avg_temp) < 2:
                na_avg = na_avg + 1
            avg_mmax_live = float(rowdict[row[0]][2])
            max_temp = float(row[3])
            if abs(avg_mmax_live - max_temp) < 2:
                na_max = na_max + 1
            avg_mmin_live = float(rowdict[row[0]][3])
            min_temp = float(row[4])
            if abs(avg_mmin_live - min_temp) < 2:
                na_min = na_min + 1
    # 把一条SQL的值加载list中，然后把这个list合并到总的list中
    #print na_avg, na_max, na_min, na_mmax, na_mmin
    cursor = db.cursor()
    sql='replace into t_r_avg_temp_est_daily_mos_dem_winter2(initial_time,forecast_days,initial_year,initial_month,initial_day,initial_hour,NN,na_avg,na_max,na_min)VALUES ("'+initial+'","'+str(ii+1)+'","'+str(year002)+'","'+str(month002)+'","'+str(day002)+'","'+str(hour002)+'","'+str(nn)+'","'+str(na_avg)+'","'+str(na_max)+'","'+str(na_min)+'")'
...
def calculate_temp3h_accurity(db,initial,pdate,odate,ii):
    #72小时气温的准确率
    cursor001 = db.cursor()
    sql = 'select city_id,initial_time,temperature from t_r_ec_city_forecast_ele_mos_dem_winter where initial_time="' + initial + '" and forecast_time>"' + pdate + '" and forecast_time<="' + odate + '"'
    print sql
    cursor001.execute(sql)
    ...
    accu=na*100/nn
    sql_into='replace into t_r_3h_temp_accu_mos_dem_winter2(initial_time,forecast_hours,temp_accu,temp_mae,temp_rmse)VALUES ("'+initial+'","'+str(ii)+'","'+str(accu)+'","'+str(mae)+'","'+str(rmse)+'")'
    print sql_into
...
def calculate_maxmintemp6h_accuracy(db, initial, pdate, odate, u):
    #预报的6h最高气温最低气温
    cursor001 = db.cursor()
    sql_mm6h = 'select city_id,initial_time,temp_max_6h,temp_min_6h from t_r_ec_city_forecast_ele_mos_dem_winter where initial_time="' + initial + '" and forecast_time>"' + pdate + '" and forecast_time<="' + odate + '" '
    #print sql_mm6h
    ...
    sql_live = 'select v01000,MAX(TEM_Max),MIN(TEM_Min) from t_r_hourly_surf_live_ele where vdate>"' + pdate + '" and vdate<="' + odate + '" and TEM_Max<80 and TEM_Min<80 group by v01000;'
    #print sql_live
    cursor = db.cursor()
    cursor.execute(sql_live)
    rows_live = cursor.fetchall()
    ...
    for row_live in rows_live:
        if row_live[0] in fore_dict.keys():
            ...
            if abs(max_live-max_fore)<=2:
                na_max=na_max+1
            if abs(min_live-min_fore)<=2:
                na_min=na_min+1
    if nn==0:
        return
    ...
    sql_mm='replace into t_r_max_min_temp_accu_mos_dem_winter2(initial_time,forecast_hours,max_temp_accu,min_temp_accu)VALUES ("'+initial+'","'+str(u)+'","'+str(accuracy_max)+'","'+str(accuracy_min)+'")'
    #print sql_mm
    cursor_mm=db.cursor()
    cursor_mm.execute(sql_mm)
    db.commit()
    
#initialtime001为日期性参数
def calculateAccuracy2(initialtime001):
    #考虑到实况数据延迟16-40个小时，所有10天的准确率和72小时的准确率都需要把初始时间往前提，不然实况数据取不到
    initialtime10days=initialtime001+datetime.timedelta(days=-12)
    initialtime72hour=initialtime001+datetime.timedelta(days=-5)
    initial10=datetime.datetime.strftime(initialtime10days,'%Y-%m-%d %H:%M:%S')
    initial72=datetime.datetime.strftime(initialtime72hour,'%Y-%m-%d %H:%M:%S')
    db = MySQLdb.connect('172.16.8.28', 'admin', 'moji_China_123', 'moge',
                         3307)
    # db = MySQLdb.connect('192.168.10.84', 'admin', 'moji_China_123', 'moge')
    for n in range(10):
        print n
        pdatetime = initialtime10days + datetime.timedelta(days=n)
        pdate = datetime.datetime.strftime(pdatetime, '%Y-%m-%d %H:%M:%S')
        odatetime = initialtime10days + datetime.timedelta(days=n + 1)
        odate = datetime.datetime.strftime(odatetime, '%Y-%m-%d %H:%M:%S')
        calculate_avg_temp_24h_from3h_accurity(db, initial10, pdate, odate, n)
    for u in range(24):
        pdatetime = initialtime72hour + datetime.timedelta(hours=3 * u)
        pdate = datetime.datetime.strftime(pdatetime, '%Y-%m-%d %H:%M:%S')
        odatetime = initialtime72hour + datetime.timedelta(hours=3 * (u + 1))
        odate = datetime.datetime.strftime(odatetime, '%Y-%m-%d %H:%M:%S')
        calculate_temp3h_accurity(db, initial72, pdate, odate, (u + 1) * 3)
    db.close()
    
if __name__ == "__main__":
    initial = '2018-08-18 00:00:00'
    initialtime = datetime.datetime.strptime(initial, '%Y-%m-%d %H:%M:%S')
    for p in range(62):
