...
def getECfilenameandpath(i,pdatetime,yearstr,monstr,daystr,hourstr,datestr):
    print i,pdatetime,yearstr,monstr,daystr,hourstr,datestr
    if i <90:
        udatetime = pdatetime + datetime.timedelta(hours=i+1)
    elif i>=90 and i<108:
        udatetime = pdatetime + datetime.timedelta(hours=90 + 3 * (i+1 - 90))
    elif i>=108 and i<124:
        udatetime = pdatetime + datetime.timedelta(hours=144 + 6 * (i+1 - 108))
    # 取文件
    print udatetime
    ...
    if uhour < 10:
        ...
        uhourstr = str(uhour)
    ecfile00 = 'D2S' + monstr + daystr + hourstr + '00' + umonthstr + udaystr + uhourstr + '001.bz2'
    ecfullpath = '/meteo/moge/data/ecmwf/' + yearstr + '/' + datestr + '/' + ecfile00
    print ecfullpath
    yearpath='/home/wlan_dev/mosdata/' + yearstr
    ...
    #逐3和逐6小时的数据下载
    ecfile003='D2D' + monstr + daystr + hourstr + '00' + umonthstr + udaystr + uhourstr + '001.bz2'
    ecfullpath003='/meteo/moge/data/ecmwf/' + yearstr + '/' + datestr + '/' + ecfile003
    print ecfile003
    if not os.path.exists(ecfullpath003):
        p = subprocess.call(
            '/home/wlan_dev/bigdata/api/start_hdfs_access.sh ' + ecfullpath003 + ' ' + topath + ' 1',
            shell=True)
...
if __name__ == "__main__":
    pdatetimestring00 = '2019-01-01 00:00:00'
