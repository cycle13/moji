...
if __name__ == "__main__":
    ...
    # 文件日志learning
    logfile = '/home/wlan_dev/log/lea1.log'
    ...
    starttime = datetime.datetime.strptime('2018-10-16 00:00:00',
                                           '%Y-%m-%d %H:%M:%S')
    ...
    sortecfilelist=sorted(ecfilelist)
    print sortecfilelist
    mospool=multiprocessing.Pool(processes=3)
    for i in range(len(sortecfilelist)):
        file=sortecfilelist[i]
        # 取文件中时间字段
        start = file[3:9]
        ...
        modelfile = '/home/wlan_dev/mos/wintermodel_gpu/temp_model'+id+'.model'
        scalefile = '/home/wlan_dev/mos/wintermodel_gpu/temp_scale'+id+'.save'
        print file,hours,modelfile,scalefile
        if os.path.exists(modelfile) and os.path.exists(scalefile):
            print file,filefullpath
            #predictmodel(file,filefullpath,modelfile,scalefile,stationlist,demdict,origintime,endtime)
            mospool.apply_async(predictmodel,args=(file,filefullpath,modelfile,scalefile,stationlist,demdict,origintime,endtime))
    mospool.close()
    mospool.join()