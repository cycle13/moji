...
def calculateAccuracy(initialtime001):
    ...
    db.close()
def calculate_temp_h_accurity(db,initial,pdate,odate,ii):
    #计算一个起始预报时次的所有预报的准确率。
    cursor001 = db.cursor()
    sql = 'select city_id,initial_time,temperature,forecast_time from t_r_ec_city_forecast_ele_mos_dem where initial_time="' + initial  + '" and forecast_time="' + odate + '"'
    print sql
    cursor001.execute(sql)
    rows_f=cursor001.fetchall()
    #print rows_f,type(rows_f)
    ...
    for p in range(31):
        ...
        for u in range(64):
            if u<48:
                ...
                calculate_temp_h_accurity(db, initial, pdate, odate, (u+1)*3)
            else:
                pdatetime=initialtime001+datetime.timedelta(hours=6*(u-48)+144)
                pdate = datetime.datetime.strftime(pdatetime, '%Y-%m-%d %H:%M:%S')
                odatetime=initialtime001+datetime.timedelta(hours=6*(u+1-48)+144)
                odate = datetime.datetime.strftime(odatetime, '%Y-%m-%d %H:%M:%S')
                print initial, pdate, odate,6*(u+1-48)+144
                calculate_temp_h_accurity(db, initial, pdate, odate, 6*(u+1-48)+144)
        db.close()
