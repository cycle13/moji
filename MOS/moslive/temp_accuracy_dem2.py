...
def calculate_avg_temp_24h_from3h_accurity(db, initial, pdate, odate, ii):
    ...
    cursor = db.cursor()
    sql='replace into t_r_avg_temp_est_daily_mos_dem2(initial_time,forecast_days,initial_year,initial_month,initial_day,initial_hour,NN,na_avg,na_max,na_min)VALUES ("'+initial+'","'+str(ii+1)+'","'+str(year002)+'","'+str(month002)+'","'+str(day002)+'","'+str(hour002)+'","'+str(nn)+'","'+str(na_avg)+'","'+str(na_max)+'","'+str(na_min)+'")'
...
def calculate_temp3h_accurity(db,initial,pdate,odate,ii):
    ...
    accu=na*100/nn
    sql_into='replace into t_r_3h_temp_accu_mos_dem2(initial_time,forecast_hours,temp_accu,temp_mae,temp_rmse)VALUES ("'+initial+'","'+str(ii)+'","'+str(accu)+'","'+str(mae)+'","'+str(rmse)+'")'
    print sql_into
...
def calculate_maxmintemp6h_accuracy(db, initial, pdate, odate, u):
    #预报的6h最高气温最低气温
    cursor001 = db.cursor()
    sql_mm6h = 'select city_id,initial_time,temp_max_6h,temp_min_6h from t_r_ec_city_forecast_ele_mos_dem where initial_time="' + initial + '" and forecast_time>"' + pdate + '" and forecast_time<="' + odate + '" '
    #print sql_mm6h
    ...
    sql_mm='replace into t_r_max_min_temp_accu_mos_dem2(initial_time,forecast_hours,max_temp_accu,min_temp_accu)VALUES ("'+initial+'","'+str(u)+'","'+str(accuracy_max)+'","'+str(accuracy_min)+'")'
    #print sql_mm
    cursor_mm=db.cursor()
...
def calculateAccuracy(initialtime001):
