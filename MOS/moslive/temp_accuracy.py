...
@app.task
def calculate_avg_temp_24h_from3h_accurity001(db, initial, pdate, odate, ii):
    ...
    db.commit()
@app.task
def calculate_temp3h_accurity001(db,initial,pdate,odate,ii):
    #72小时气温的准确率
    cursor001 = db.cursor()
...
def calculateAccuracy001(initialtime001):
    ...
    for n in range(10):
        ...
        calculate_avg_temp_24h_from3h_accurity001(db, initial10, pdate, odate, n)
    for u in range(24):
        pdatetime = initialtime72hour + datetime.timedelta(hours=3 * u)
        pdate = datetime.datetime.strftime(pdatetime, '%Y-%m-%d %H:%M:%S')
        odatetime = initialtime72hour + datetime.timedelta(hours=3 * (u + 1))
        odate = datetime.datetime.strftime(odatetime, '%Y-%m-%d %H:%M:%S')
        calculate_temp3h_accurity001(db, initial72, pdate, odate, (u + 1) * 3)
    db.close()
