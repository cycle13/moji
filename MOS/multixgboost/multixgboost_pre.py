...
def GetOnetimeFromEC(n,sfc_varinames,sfc_file,indexlat,indexlon):
    ...
    for i in range(len(sfc_varinames)):
        ...
        vstring.append(latlonArray[indexlat][indexlon - 1])
    return vstring
def GetStationsAndOnetimesFromEC(i,sfc_varinames,sfc_file,inputfile,stationsVlist,trainlebellist,stationdict,stationlist,dict01):
...
def modelprocess(stationdict,stationlist,ll):
    sys.stdout=open(os.path.join(outpath,'pre'+str(ll)+'.out'),'w')
    print outpath
    ...
    for rootpath, dirs, files in os.walk(allpath):
        for file in files:
            if file[:3] == 'sfc' and file[-5:] == '.grib':
                ...
                GetStationsAndOnetimesFromEC(ll,sfc_varinames,sfcfile,inputfile,stationsVlist,trainlebellist,stationdict,stationlist,dict01)
    #训练集
    ...
    print rmse0,mae0,rmse,mae,len(y_train),len(y_test)
    model.save_model('/home/wlan_dev/model/pre'+str(ll)+'.model')
    testfile=os.path.join(outpath,'pretest'+str(ll)+'.csv')
    predsfile=os.path.join(outpath,'prepreds'+str(ll)+'.csv')
    originfile=os.path.join(outpath,'preorigin'+str(ll)+'.csv')
            ...
            if stationfile[i][-4:] == '.csv':
                ...
                while True:
                    ...
                    if len(linearray) > 4:
                        ...
                        if float(linearray[5]) < 9999 or float(linearray[5]) <> None:
        ...
        for ii in range(len(pstationfile)):
            if pstationfile[ii][-4:]=='.csv':
                ...
                if len(plinearray)>4:
                    ...
                    if float(plinearray[5])<9999 or float(plinearray[5])<>None:
