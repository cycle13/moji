    ...
    for p in range(len(pl_varinames)):
        ...
        for k in range(len(phaArray)):
            ...
            if pha == 500 or pha == 850:
                ...
                vstring.append(llArray[indexlat][indexlon - 1])
    return vstring
def InterpolateGetOnetimeFromEC(n, sfc_varinames, sfc_file, longitude, latitude,
                     pl_varinames, pl_file):
    vstring = []
    levelArray = pl_file.variables['lv_ISBL1']
    print sfc_varinames
    for i in range(len(sfc_varinames)):
        ...
        grid_info=get_nearest_point(longitude,latitude,60,60,0.125)
        print grid_info
        variablevalue=interpolate_grid_to_city(latlonArray,0,*grid_info)
        vstring.append(variablevalue)
    for p in range(len(pl_varinames)):
        pl_variArray = pl_file.variables[pl_varinames[p]]
        phaArray = pl_variArray[n]
        for k in range(len(phaArray)):
            ...
            if pha == 500 or pha == 850:
                grid_info = get_nearest_point(longitude, latitude, 60, 60,0.125)
                variablevalue1=interpolate_grid_to_city(llArray,0,*grid_info)
                vstring.append(variablevalue1)
    return vstring
def get_nearest_point(longitude, latitude,origin_lon,termination_lat,cellsize):
    #获取城市所在格子的四个角的顶点
    print longitude, latitude,origin_lon,termination_lat,cellsize
    indexlon=int(longitude-origin_lon)/cellsize
    indexlat=int(termination_lat-latitude)/cellsize
    print indexlon,indexlat
    lon1=origin_lon+cellsize*(indexlon)
    lat1=termination_lat-cellsize*(indexlat)
    lon2=origin_lon+cellsize*(indexlon+1)
    lat2=termination_lat-cellsize*(indexlat)
    lon3=origin_lon+cellsize*(indexlon+1)
    lat3=termination_lat-cellsize*(indexlat+1)
    lon4=origin_lon+cellsize*(indexlon)
    lat4=termination_lat-cellsize*(indexlat+1)
    print indexlon,indexlat
    # 计算城市位置附近四个格点与城市之间的距离
    r1 = get_spherical_distance(latitude, longitude, lat1,
                                lon1)
    r2 = get_spherical_distance(latitude, longitude, lat2,
                                lon2)
    r3 = get_spherical_distance(latitude, longitude, lat3,
                                lon4)
    r4 = get_spherical_distance(latitude, longitude, lat4,
                                lon4)
    #r这里用的是对角线的距离（存疑）；我直接对角的两个点的距离
    r=get_spherical_distance(lat1,lon1,lat3,lon3)
    # 计算距离平方和
    cq = (r - r1) ** 2 + (r - r2) ** 2 + (r - r3) ** 2 + (r - r4) ** 2
    cq = 1.0 / cq
    print indexlat, indexlon, r1, r2, r3, r4, cq, r
    return indexlat, indexlon, r1, r2, r3, r4, cq, r
...
def interpolate_grid_to_city(_var_grid, _mode, *_nearest_point_info):
    print _var_grid, _mode, _nearest_point_info
    indexlat, indexlon, r1, r2, r3, r4, cq, r = _nearest_point_info
    if _mode == 0:
        # 反距离加权插值
        print 'value',len(_var_grid),
        _v1 = _var_grid[int(indexlat)][int(indexlon)] * (r - r1) ** 2
        _v2 = _var_grid[int(indexlat)+1][int(indexlon)] * (r - r2) ** 2
        _v3 = _var_grid[int(indexlat)][int(indexlon)+1] * (r - r3) ** 2
        _v4 = _var_grid[int(indexlat)+1][int(indexlon)+1] * (r - r4) ** 2
        ...
        k = l.index(min(l))
        if k == 0:
            return _var_grid[indexlat][indexlon]
        elif k == 1:
            return _var_grid[indexlat+1][indexlon]
        elif k == 2:
            return _var_grid[indexlat][indexlon+1]
        else:
            return _var_grid[indexlat+1][indexlon+1]
def GetStationsAndOnetimesFromEC(i, sfc_varinames, sfc_file, pl_varinames,
    ...
    for j in range(len(stationlist)):
        ...
        if trainlebel <> None and trainlebel <> 999999:
            ...
            perstationlist= InterpolateGetOnetimeFromEC(i, sfc_varinames, sfc_file,longitude,latitude,pl_varinames, pl_file)
...
def modelprocess(stationdict,stationlist,ll):
    ...
    fh=logging.FileHandler(os.path.join(outpath,'t_t'+str(ll)+'.log'))
    fh.setLevel(logging.INFO)
    consolehand=logging.StreamHandler()
    consolehand.setLevel(logging.INFO)
    ...
    print("训练后MAE: %.4f" % mae)
    logger.info("训练后MSE: %.4f" % mse)
    logger.info("训练后RMSE: %.4f" % rmse)
    logger.info("训练后MAE: %.4f" % mae)
    #气温差2度的准确率
    n=0
    ...
    logger.info("训练后2度的accuracy: %.4f" % accuracy2_after)
    n=0
    for x,y in zip(y_test,preds):
        if abs(x-y)<3:
            n=n+1
    ...
    logger.info("训练后3度的accuracy: %.4f" % accuracy3_after)
    #和EC中原始数据对比获取均方误差
    y_origin=a_test[:,0]
    #print y_origin
    ...
    logger.info("训练前MSE: %.4f" % mse0)
    logger.info("训练前RMSE: %.4f" % rmse0)
    logger.info("训练前MAE: %.4f" % mae0)
    n=0
    ...
    logger.info("训练前2度的accuracy: %.4f" % accuracy2_before)
    n=0
    for x,y in zip(y_test,y_origin):
        if abs(x-y)<3:
            n=n+1
    ...
    logger.info("训练前3度的accuracy: %.4f" % accuracy3_before)
    ...
    logger.info(str(rmse0)+','+str(mae0)+','+str(accuracy2_before)+','+str(accuracy3_before)+','+str(rmse)+','+str(mae)+','+str(accuracy2_after)+','+str(accuracy3_after)+','+str(len(a_train))+','+str(len(a_test)))
