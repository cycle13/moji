...
for i in range(92):
    ...
    filename = '/home/wlan_dev/maxmin/maxminmy' + datetime.datetime.strftime(
        odatetime, '%Y%m%d') + '.csv'
    ...
    sql='select v01000, max(v_TEM),min(v_TEM) from t_r_hourly_surf_live_ele where VDate>= " '+odatetime0+'" and VDate< "'+pdatetime0+'" and v_TEM<>999999 GROUP BY v01000'
    ...
    for row in rows:
        ...
        csvfile.write(str(row[0])+','+str(row[1])+','+str(row[2]))
        csvfile.write('\n')
    csvfile.close()
db.close()
