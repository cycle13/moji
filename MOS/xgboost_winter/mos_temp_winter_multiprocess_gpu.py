...
if __name__=='__main__':
    ...
    # 文件日志learning
    # file_handler = logging.handlers.RotatingFileHandler(logfile,maxBytes=500 * 1024 * 1024,backupCount=7)
    ...
    outpath='/home/meteo/yetao.lu/mos'
    ...
    for ecroot,dirs,files in os.walk(ecpath):
        for file in files:
            if file[:3] == 'sfc' and file[-5:] == '.grib' and (file[8:10]<>'06' and file[8:10]<>'07' and file[8:10]<>'08'):
                ...
                if not os.path.exists(ecfile_sfc) and not os.path.exists(ecfile_pl):
                    continue
                else:
    ...
    # for aa in range(len(trainarray)):
    #     logger.info(trainarray[aa])
    #     logger.info('\n')
    #对样本因子集进行分割
    ...
    originfw.close()
    time004=time.time()
    logger.info('数据采集时间:'+str((time002-time001)/60)+'模型训练：'+str((time003-time002)/60)+'准确率计算数据保存：'+str((time004-time003)/60))