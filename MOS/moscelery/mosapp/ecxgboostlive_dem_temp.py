...
总降水也要减去1.87进行预测
"""
import datetime, os, xgboost, numpy, bz2, shutil,subprocess,multiprocessing, sys, MySQLdb, pygrib, logging,time
...
def demdatefromcsvTodict(demcsv):
    ...
    return dictdem
@app.task
def calculateStationVariable(tempvariablelist, inputfile,
    ...
    if inputfile[-3:] == '001' :
        ...
        grbs.close()
@app.task
def Predict( outfilename, modelname, tempscalerfile, origintime,
            foretime,csvfile, demcsv):
    try:
        # 取气温、最高气温、最低气温的训练训练矩阵。读文件费劲，连降水一起取了。晴雨、降水一块训
        logger.info('----------------------------------------------')
        tempvariablelist = []
        stationlist = []
        calculateStationVariable(tempvariablelist,
        ...
        # logger.info(result)
        #
        db = MySQLdb.connect('172.16.8.28', 'admin', 'moji_China_123',
                             'moge', 3307)
        # db = MySQLdb.connect('192.168.10.84', 'admin', 'moji_China_123','moge')
        cursor = db.cursor()
        origin = datetime.datetime.strftime(origintime,
                                            '%Y-%m-%d %H:%M:%S')
        forecast = datetime.datetime.strftime(foretime,
                                              '%Y-%m-%d %H:%M:%S')
...
#starttime是系统时间
#filename为单个EC文件名称
def simplemodelProdict(filepath,tofilepath,yearint):
    #为了把逻辑变得简单，复制文件，解压都放在这里了，
    shutil.copy(filepath,tofilepath)
    logger.info(filepath+'-----'+tofilepath)
    filename001=os.path.split(filepath)[1]
    filefullname=os.path.join(tofilepath,filename001)
    subprocess.call('bzip2 -d -k ' + filefullname, shell=True)
    #filename是文件的全路径
    logger.info('>>>>>>>>>>>>>>>>>>>>>')
    logger.info(filefullname)
    onlyname = filename001
    #解压后的文件全路径
    unzipfilename=filefullname[:-4]
    start = onlyname[3:9]
    end = onlyname[11:17]
    print start,end
    start001 = str(yearint) + start
    end001 = str(yearint) + end
    if start001 > end001:
        end001 = str(yearint + 1) + end
    origintime = datetime.datetime.strptime(start001,'%Y%m%d%H')
    endtime = datetime.datetime.strptime(end001, '%Y%m%d%H')
    d = (endtime - origintime).days
    hours = (d * 24 + (endtime - origintime).seconds / 3600)
    if hours <= 144 and hours > 0:
        id = hours / 3
    elif hours > 144:
        id = 48 + (hours - 144) / 6
    ...
    Predict(unzipfilename, modelname, tempscalerfile, origintime, endtime, csvfile, demcsv)
...
#         os.makedirs(outpath)
#     modelProdict('/mnt/data/ecdata/2018/2018-07-09', outpath, csvfile,
#                  demcsv, starttime)
