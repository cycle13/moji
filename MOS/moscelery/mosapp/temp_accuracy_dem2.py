...
def calculate_maxmintemp6h_accuracy(db, initial, pdate, odate, u):
    ...
    db.commit()
#initialtime001为字符串参数，日期对象在消息传输的时候是不认的,插入的数据是实时数据，需要转成00和12时
def calculateAccuracy_2(initial_time0012):
    #考虑到实况数据延迟16-40个小时，所有10天的准确率和72小时的准确率都需要把初始时间往前提，不然实况数据取不到
    initialtime001=datetime.datetime.strptime(initial_time0012,'%Y-%m-%d %H:%M:%S')
    initialtime10days=initialtime001+datetime.timedelta(days=-13)
    initialtime72hour=initialtime001+datetime.timedelta(days=-6)
